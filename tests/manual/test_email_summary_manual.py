...
def test_real_email_summary_tool():
    ...
    try:
        ...
        if not connection_result.get("success", False):
            print(f"❌ Gmail connection failed: {connection_result.get('message', 'Unknown error')}")
            return False
...
def interactive_test():
    ...
    try:
        ...
        import mcp_server
        result = mcp_server.send_email_summary(recipient_email=recipient, max_emails=max_emails)
...
if __name__ == "__main__":
    ...
    cred_files = ["credentials.json", str(Path.home() / ".local/fastmcp_gmail/credentials.json")]
