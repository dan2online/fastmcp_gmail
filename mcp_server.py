    ...
    try:
        ...
        for email in emails:
            simplified_email = {
                ...
                "content_preview": email.get("content", {}).get("text", "")[:500] + "..." if len(email.get("content", {}).get("text", "")) > 500 else email.get("content", {}).get("text", ""),
                "is_unread": email.get("metadata", {}).get("is_unread", False),
                "has_attachments": email.get("metadata", {}).get("has_attachments", False),
...
def search_emails(query: str, max_results: int = 50) -> List[dict]:
    ...
    """
    try:
        logger.info(f"Searching emails with query: '{query}', max results: {max_results}")
        service, reader = ensure_gmail_connection()
        ...
        for email in emails:
            simplified_email = {
                ...
                "sender": email.get("sender", "(Unknown)"),
                "date": str(email.get("date")) if email.get("date") else "Unknown",
                "content_preview": email.get("content", {}).get("text", "")[:300] + "..." if len(email.get("content", {}).get("text", "")) > 300 else email.get("content", {}).get("text", ""),
                "is_unread": email.get("metadata", {}).get("is_unread", False),
                "has_attachments": email.get("metadata", {}).get("has_attachments", False),
                "importance": email.get("metadata", {}).get("importance", "normal")
            }
...
def send_email_summary(recipient_email: str = "me", max_emails: int = 10) -> dict:
    ...
    try:
        ...
        logger.info(f"Successfully sent email summary with {len(summaries)} emails to {recipient_email}")
